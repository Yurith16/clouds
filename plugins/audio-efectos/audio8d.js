...
export default {
  ...
  execute: async (sock, msg, { from }) => {
    ...
    try {
      ...
      if (!/audio|video/.test(mime)) return sock.sendMessage(from, { text: '> 🎧 Responde a un audio o video para aplicar efecto 8D 🍃' }, { quoted: msg })
      ...
      const inputPath = path.join(TEMP_DIR, `in_8d_${Date.now()}`)
      const outputPath = path.join(TEMP_DIR, `out_8d_${Date.now()}.mp3`)
      ...
      await new Promise((resolve, reject) => {
        ffmpeg(inputPath)
          .audioFilters([
            // apulsator mueve el audio de un lado a otro. 
            // hz=0.125 es la velocidad de rotación ideal para no marear.
            'apulsator=hz=0.125' 
          ])
