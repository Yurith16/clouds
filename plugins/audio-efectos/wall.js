...
export default {
  ...
  execute: async (sock, msg, { from }) => {
    ...
    try {
      ...
      if (!/audio|video/.test(mime)) return sock.sendMessage(from, { text: '> 🎙️ Responde a un audio o video para que suene "detrás de la pared" 🍃' }, { quoted: msg })
      ...
      const inputPath = path.join(TEMP_DIR, `in_wall_${Date.now()}`)
      const outputPath = path.join(TEMP_DIR, `out_wall_${Date.now()}.mp3`)
      ...
      await new Promise((resolve, reject) => {
        ffmpeg(inputPath)
          .audioFilters([
            // 1. Filtro Lowpass: Corta todo lo que esté por encima de 450Hz
            // Esto simula que el sonido atraviesa una pared sólida.
            'lowpass=f=450',
            // 2. Un poco de Reverb ligero para simular el eco del cuarto donde estás tú
            'aecho=1.0:0.5:20:0.3',
            // 3. Subimos el volumen final un poco porque el filtro lowpass lo apaga mucho
            'volume=1.8'
          ])
