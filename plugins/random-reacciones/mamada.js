...
export default {
  ...
  execute: async (sock, msg, { from }) => {
    ...
    const usedCommand = textMsg.split(' ')[0].slice(1) || 'blowjob'
    ...
    if (!targetJid) {
      const helpText = `text: "> *Etiqueta a quien te debe un buen servicio hoy...* 🤭🔥\n\n` +
    ...
    try {
      ...
      // 6. Mensaje con roles invertidos: el etiquetado le da el servicio al que usa el comando
      const txt = `*¡Qué traviesos!* @${victimJid.split('@')[0]} le está dando un buen servicio a @${selfJid.split('@')[0]} 🤭🔥`
