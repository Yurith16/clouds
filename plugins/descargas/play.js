...
export default {
  ...
  async execute(sock, msg, { args, from }) {
    ...
    try {
      ...
      const { title, dl_url, thumb, size, sizeB } = res
      const finalTitle = title || videoTitle
      
      const sizeMB = (sizeB / 1024 / 1024).toFixed(2)
      
      if (sizeB > 300 * 1024 * 1024) {
        await sock.sendMessage(from, { text: `> ⚠️ Audio demasiado grande (${sizeMB}MB). Límite 300MB`, edit: processingMsg.key })
        return
      }
      ...
      await sock.sendMessage(from, {
        ...
        mimetype: 'audio/mpeg',
        fileName: `${finalTitle.substring(0, 50).replace(/[<>:"/\\|?*]/g, '')}.mp3`,
        caption: `${finalUrl}`,
        contextInfo: {
          externalAdReply: {
